...
확정(사용자 제안, 채택). 체크하면 이벤트 문서에 `isDday: true` 필드 저장. 홈 화면에서 `isDday===true`인
이벤트들을 모아 D-day 카드로 나열 — 과거 날짜면 D+N, 미래 날짜면 D-N. 미래 디데이는 여행 D-day(D-3/D-1)와
같은 패턴으로 알림 확장 가능(세부는 착수 시 재논의).

### 3. 그날의 우리 (기존 기능 확장 — 신규 아님, 착수 전 반드시 이 문단부터 읽을 것)
**`Home.jsx`에 이미 존재하는 기능**(`yearAgoEvents`, 정확히 1년 전 ±3일, `events` 컬렉션만, 최대 2개 표시)을
확장하는 작업. 새 기능으로 착각해서 처음부터 다시 만들지 말 것. 확장 범위:
- 1년 전뿐 아니라 2년/3년 전도 확인
- `events`뿐 아니라 `personal_events`도 포함
- 카드 디자인 개선(존재감 강화 — 사용자가 "지금은 존재감이 없다"고 지적함)

**알림은 추가하지 않기로 확정** — 일정을 자주 등록하는 커플은 1년 후 거의 매일 매칭이 발생해서, "가끔 오는
반가운 알림"이 아니라 사실상 매일 오는 알림이 되어버림(사용자가 직접 지적, 타당함). 홈 화면 진입 시
"발견하는" 형태로만 유지.

사진은 이벤트 이미지 업로드 기능(위 "남은 작업" 참고, 아직 미구현)이 선행돼야 표시 가능 — 그 전까지는
제목+날짜 텍스트만.
